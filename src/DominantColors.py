...
CLUSTERS = args.clusters if args.clusters else 8
WIDTH = 128
HEIGHT = 128
...
def plotColorClusters(img):
    ...
    # print(mydf) # Descomentar
    nuevo = mydf[['color_name']].to_numpy()
    porcentaje = mydf[['Percentage']].to_numpy()
    # print(f'\n\narray es {nuevo[0][0]}')
...
def main(nombre):
    a = args.imagepath if args.imagepath else "uploads/" + nombre
    img = Image.open(a)
    return plotColorClusters(img)
    
# if __name__ == '__main__':
#     inicio = time.time()
#     main()
#     print(f'ejecucion es: {time.time() - inicio}')